...
class LinearNormalGamma(nn.Module):
    ...
    def forward(self, x):
        ...
        mu, logv, logalpha, logbeta = [
            w.squeeze(-1) for w in torch.split(pred, 1, dim=-1)
        ]
        return (
            mu,
            self.evidence(logv),
            self.evidence(logalpha) + 1,
            self.evidence(logbeta),
        )
...
class DNN(torch.nn.Module):
        ...
        if len(block_sizes) > 0:
            ...
            if len(block_sizes) > 1:
                for i in range(len(block_sizes) - 1):
                    blocks += self.block(
                        block_sizes[i], block_sizes[i + 1], dr[i], batch_norm
                    )
            if lng:
                blocks.append(LinearNormalGamma(block_sizes[-1], output_size))
            else:
                blocks.append(
                    SpectralNorm(torch.nn.Linear(block_sizes[-1], output_size))
                )
        else:
            if lng:
    ...
    def block(self, input_size, output_size, dr, batch_norm):
        block = [SpectralNorm(torch.nn.Linear(input_size, output_size))]
        if batch_norm:
        ...
        if self.lng:
            ...
            with torch.no_grad():
                if batch_size < x.shape[0]:
                    ...
                    pred = self.forward(torch.from_numpy(x).float().to(device)).cpu()
